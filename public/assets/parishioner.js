...
.member-legion-create label>i:first-child,.member-legion-create legend>i:first-child,.member-legion-edit label>i:first-child{margin-right:2px;color:#dc4350;font-style:normal}
    ...
    const timer3 = window.setInterval(() => {
      ...
      if (tab && panel2) {
        ...
        if (target === "prayer-dream") {
          const launcher = [...document.querySelectorAll(".sharing-registration-launcher")].find((item) => item.querySelector("button")?.textContent?.includes("\uAE30\uB3C4\uBB38"));
          if (launcher) {
            launcher.hidden = false;
            launcher.classList.add("parish-host-compose-launcher");
            main.append(launcher);
          }
        }
        main.append(panel2);
  ...
  document.head.insertAdjacentHTML("beforeend", `<style>
.parish-host-user{display:flex;align-items:center;justify-content:flex-end;gap:10px}.parish-host-user>a{max-width:240px;overflow:hidden;color:#fff;font-size:12px;font-weight:800;text-decoration:underline;text-decoration-color:rgba(255,255,255,.45);text-underline-offset:4px;white-space:nowrap;text-overflow:ellipsis}.parish-host-user>a:hover,.parish-host-user>a:focus-visible{color:#f5d88b;text-decoration-color:#f5d88b}.parish-host-user>button{white-space:nowrap}.parish-host-compose-launcher{display:flex!important;justify-content:flex-end;margin:0 0 14px}.parish-host-compose-launcher .registration-form-open{width:auto;min-width:150px}body.parish-host-feature-open .member-profile-modal,body.parish-host-feature-open .registration-form-modal,body.parish-host-feature-open>.member-modal{z-index:2147483647}@media(max-width:640px){.parish-host-user{grid-column:1/-1;width:100%}.parish-host-user>a{margin-left:auto}.parish-host-feature>header .parish-host-user>button{grid-column:auto;margin-top:0}}
</style>`);
  document.head.insertAdjacentHTML("beforeend", `<style>
