...
app.post("/api/parish-registration/code", async (req, res, next) => {
  try {
    ...
    await pool.execute(
      "INSERT INTO parish_registration_codes (manager_name, email, code_hash, expires_at) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE))",
...
app.get("/api/parishioner/videos", requireParishioner, async (_req, res, next) => {
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      "SELECT id, youtube_url AS youtubeUrl, video_id AS videoId, title, author_name AS authorName, thumbnail_url AS thumbnailUrl, created_at AS createdAt FROM parish_videos WHERE parish_id = ? ORDER BY created_at DESC, id DESC",
      [parishId],
    );
...
app.get("/api/parishioner/shrines",requireParishioner,async(_req,res,next)=>{try{const userId=await currentParishionerId(res);const [rows]=await pool.query<RowDataPacket[]>(`SELECT s.id,s.diocese,s.name,s.address,s.website_url AS websiteUrl,v.id AS visitId,DATE_FORMAT(v.visited_at,'%Y-%m-%d') AS visitedDate,(SELECT COUNT(*) FROM shrine_visit_photos p WHERE p.visit_id=v.id AND p.enabled=1) AS photoCount FROM catholic_shrines s LEFT JOIN shrine_pilgrim_visits v ON v.shrine_id=s.id AND v.parishioner_id=? WHERE s.enabled=1 ORDER BY v.visited_at DESC,s.diocese,s.source_order,s.name`,[userId]);res.json(rows.map(row=>({...row,id:Number(row.id),visitId:row.visitId?Number(row.visitId):null,visited:Boolean(row.visitId),photoCount:Number(row.photoCount)})))}catch(error){next(error)}});
...
app.get("/api/parishioner/prayer-dream/recipients",requireParishioner,async(req,res,next)=>{
  try{
    ...
    const [rows]=await pool.query<RowDataPacket[]>("SELECT id,name,baptismal_name AS baptismalName,email FROM parishioners WHERE parish_id=? AND id<>? AND (name LIKE ? OR baptismal_name LIKE ? OR email LIKE ?) ORDER BY name LIMIT 20",[parishId,userId,q,q,q]);
    res.json(rows.map(row=>({...row,id:Number(row.id)})));
  }catch(error){next(error)}
});
...
app.post("/api/parish-auth/code", async (req, res, next) => {
  try {
    ...
    const hash = crypto.createHash("sha256").update(code).digest("hex");
    await pool.execute(
      "INSERT INTO parish_login_codes (parish_id, email, code_hash, expires_at) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE))",
      [parishId, email, hash],
    );
...
app.get("/api/parish/videos", requireParish, async (_req, res, next) => { try { const parishId = Number(res.locals.parishSession.parish_id); const [rows] = await pool.query<RowDataPacket[]>("SELECT id, youtube_url AS youtubeUrl, video_id AS videoId, title, author_name AS authorName, thumbnail_url AS thumbnailUrl, created_at AS createdAt FROM parish_videos WHERE parish_id = ? ORDER BY created_at DESC, id DESC", [parishId]); res.json(rows); } catch (error) { next(error); } });
app.post("/api/parish/videos", requireParish, async (req, res, next) => { try { const parishId = Number(res.locals.parishSession.parish_id), metadata = await youtubeMetadata(req.body.url); const [result] = await pool.execute<mysql.ResultSetHeader>("INSERT INTO parish_videos (parish_id, youtube_url, video_id, title, author_name, thumbnail_url) VALUES (?, ?, ?, ?, ?, ?)", [parishId, metadata.youtubeUrl, metadata.videoId, metadata.title, metadata.authorName || null, metadata.thumbnailUrl]); res.status(201).json({ message: "동영상이 등록되었습니다.", id: result.insertId }); } catch (error) { if ((error as { code?: string }).code === "ER_DUP_ENTRY") return res.status(409).json({ message: "이미 등록된 동영상입니다." }); next(error); } });
