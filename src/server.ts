...
app.post("/api/supervisor/auth/code", async (req, res, next) => {
  try {
    ...
    const supervisorEmail = normalizeEmail(process.env.SUPERVISOR_EMAIL);
    const expectedPassword = process.env.SUPERVISOR_PASSWORD ?? "";
    ...
    if (!supervisorEmail || email !== supervisorEmail) return res.status(403).json({ message: "입력된 정보를 확인해주세요." });
...
app.get("/api/parishioner/legion/organizations/:id/inquiries",requireParishioner,async(req,res,next)=>{try{const id=Number(req.params.id),parishId=Number(res.locals.parishioner.parish_id),personId=await currentParishionerId(res),officer=await praesidiumOfficerAccess(res,id);const [organizations]=await pool.query<RowDataPacket[]>("SELECT id FROM legion_organizations WHERE id=? AND parish_id=? AND organization_type='praesidium' AND status='approved'",[id,parishId]);if(!organizations.length)return res.status(404).json({message:"활동 중인 쁘레시디움을 찾을 수 없습니다."});const [rows]=await pool.query<RowDataPacket[]>(`SELECT i.id,i.content,i.response,i.created_at AS createdAt,i.responded_at AS respondedAt,a.name AS authorName,a.baptismal_name AS authorBaptismalName,r.name AS responderName,r.baptismal_name AS responderBaptismalName,(i.author_id=?) AS isAuthor FROM legion_inquiries i JOIN parishioners a ON a.id=i.author_id LEFT JOIN parishioners r ON r.id=i.responder_id WHERE i.organization_id=? AND (i.author_id=? OR i.responder_id=? OR (?=1 AND i.responder_id IS NULL)) ORDER BY i.created_at DESC`,[personId,id,personId,personId,officer?1:0]);res.json(rows.map(row=>({...row,id:Number(row.id),isAuthor:Boolean(row.isAuthor)})))}catch(error){next(error)}});
...
app.get("/api/parishioner/legion/organizations/:id/applications",requireParishioner,async(req,res,next)=>{try{const id=Number(req.params.id),access=await legionLeaderAccess(res,id);if(!access)return res.status(403).json({message:"단장만 가입 요청을 확인할 수 있습니다."});const [rows]=await pool.query<RowDataPacket[]>("SELECT a.id,a.motivation,a.status,a.rejection_reason AS rejectionReason,a.requested_at AS requestedAt,p.name,p.baptismal_name AS baptismalName,p.email FROM legion_membership_applications a JOIN parishioners p ON p.id=a.applicant_id WHERE a.organization_id=? ORDER BY FIELD(a.status,'requested','rejected','approved'),a.requested_at DESC",[id]);res.json(rows.map(row=>({...row,id:Number(row.id)})))}catch(error){next(error)}});
...
app.get("/api/parish/legion/organizations",requireParish,async(_req,res,next)=>{try{const parishId=Number(res.locals.parishSession.parish_id);const [rows]=await pool.query<RowDataPacket[]>(`SELECT o.id,o.parent_id AS parentId,o.organization_type AS organizationType,o.name,o.description,o.status,o.icon_data IS NOT NULL AS hasIcon,o.created_at AS createdAt,o.ended_at AS endedAt,parent.name AS parentName,
  ...
  FROM legion_organizations o LEFT JOIN legion_organizations parent ON parent.id=o.parent_id LEFT JOIN legion_members m ON m.organization_id=o.id LEFT JOIN parishioners p ON p.id=m.parishioner_id
